...
def main(args):
    ...
    use_median_filter = ('median_filter' in args.reg_types.keys())
    # Create dataset object (loads metadata)
    if libwallerlab:
        dataset = Dataset(dataset_full_path, use_median_filter=use_median_filter, subtract_mean_dark_current=False, force_type='motion_deblur')
    else:
        dataset = MotionDeblurDataset(dataset_path=dataset_full_path, use_median_filter=use_median_filter, subtract_mean_dark_current=False, force_type='')
    force = False
...
def parse():
    ...
    parser.add_argument('--nit', type=int, default=100)
